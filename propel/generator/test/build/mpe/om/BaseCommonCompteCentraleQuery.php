 ...
 *
 * @method CommonCompteCentraleQuery leftJoinCommonCentralePublication($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonCentralePublication relation
 * @method CommonCompteCentraleQuery rightJoinCommonCentralePublication($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonCentralePublication relation
 * @method CommonCompteCentraleQuery innerJoinCommonCentralePublication($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonCentralePublication relation
 *
...
abstract class BaseCommonCompteCentraleQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @see       filterByCommonCentralePublication()
     *
    ...
    /**
     * Filter the query by a related CommonCentralePublication object
     *
     * @param   CommonCentralePublication|PropelObjectCollection $commonCentralePublication The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonCompteCentraleQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonCentralePublication($commonCentralePublication, $comparison = null)
    {
        if ($commonCentralePublication instanceof CommonCentralePublication) {
            return $this
                ->addUsingAlias(CommonCompteCentralePeer::ID_CENTRALE, $commonCentralePublication->getId(), $comparison);
        } elseif ($commonCentralePublication instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(CommonCompteCentralePeer::ID_CENTRALE, $commonCentralePublication->toKeyValue('Id', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByCommonCentralePublication() only accepts arguments of type CommonCentralePublication or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonCentralePublication relation
     *
     ...
     * @return CommonCompteCentraleQuery The current query, for fluid interface
     */
    public function joinCommonCentralePublication($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonCentralePublication');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonCentralePublication');
        }
    ...
    /**
     * Use the CommonCentralePublication relation CommonCentralePublication object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonCentralePublicationQuery A secondary query class using the current class as primary query
     */
    public function useCommonCentralePublicationQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinCommonCentralePublication($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonCentralePublication', 'CommonCentralePublicationQuery');
    }
