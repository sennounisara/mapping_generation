 ...
 *
 * @method CommonTypeProcedureOrganismeQuery leftJoinCommonTCalendrierEtapeReferentiel($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonTCalendrierEtapeReferentiel relation
 * @method CommonTypeProcedureOrganismeQuery rightJoinCommonTCalendrierEtapeReferentiel($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonTCalendrierEtapeReferentiel relation
 * @method CommonTypeProcedureOrganismeQuery innerJoinCommonTCalendrierEtapeReferentiel($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonTCalendrierEtapeReferentiel relation
 *
...
abstract class BaseCommonTypeProcedureOrganismeQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related CommonTCalendrierEtapeReferentiel object
     *
     * @param   CommonTCalendrierEtapeReferentiel|PropelObjectCollection $commonTCalendrierEtapeReferentiel  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonTypeProcedureOrganismeQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonTCalendrierEtapeReferentiel($commonTCalendrierEtapeReferentiel, $comparison = null)
    {
        if ($commonTCalendrierEtapeReferentiel instanceof CommonTCalendrierEtapeReferentiel) {
            return $this
                ->addUsingAlias(CommonTypeProcedureOrganismePeer::ID_TYPE_PROCEDURE, $commonTCalendrierEtapeReferentiel->getIdTypeProcedure(), $comparison)
                ->addUsingAlias(CommonTypeProcedureOrganismePeer::ORGANISME, $commonTCalendrierEtapeReferentiel->getOrganisme(), $comparison);
        } else {
            throw new PropelException('filterByCommonTCalendrierEtapeReferentiel() only accepts arguments of type CommonTCalendrierEtapeReferentiel');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonTCalendrierEtapeReferentiel relation
     *
     ...
     * @return CommonTypeProcedureOrganismeQuery The current query, for fluid interface
     */
    public function joinCommonTCalendrierEtapeReferentiel($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonTCalendrierEtapeReferentiel');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonTCalendrierEtapeReferentiel');
        }
    ...
    /**
     * Use the CommonTCalendrierEtapeReferentiel relation CommonTCalendrierEtapeReferentiel object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonTCalendrierEtapeReferentielQuery A secondary query class using the current class as primary query
     */
    public function useCommonTCalendrierEtapeReferentielQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonTCalendrierEtapeReferentiel($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonTCalendrierEtapeReferentiel', 'CommonTCalendrierEtapeReferentielQuery');
    }
