 ...
 *
 * @method CommonAvenantQuery leftJoinCommonContrat($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonContrat relation
 * @method CommonAvenantQuery rightJoinCommonContrat($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonContrat relation
 * @method CommonAvenantQuery innerJoinCommonContrat($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonContrat relation
 *
...
abstract class BaseCommonAvenantQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related CommonContrat object
     *
     * @param   CommonContrat|PropelObjectCollection $commonContrat The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonAvenantQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonContrat($commonContrat, $comparison = null)
    {
        if ($commonContrat instanceof CommonContrat) {
            return $this
                ->addUsingAlias(CommonAvenantPeer::ID_CONTRAT, $commonContrat->getIdContrat(), $comparison);
        } elseif ($commonContrat instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(CommonAvenantPeer::ID_CONTRAT, $commonContrat->toKeyValue('IdContrat', 'IdContrat'), $comparison);
        } else {
            throw new PropelException('filterByCommonContrat() only accepts arguments of type CommonContrat or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonContrat relation
     *
     ...
     * @return CommonAvenantQuery The current query, for fluid interface
     */
    public function joinCommonContrat($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonContrat');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonContrat');
        }
    ...
    /**
     * Use the CommonContrat relation CommonContrat object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonContratQuery A secondary query class using the current class as primary query
     */
    public function useCommonContratQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonContrat($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonContrat', 'CommonContratQuery');
    }
