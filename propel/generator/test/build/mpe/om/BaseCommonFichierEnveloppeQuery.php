 ...
 *
 * @method CommonFichierEnveloppeQuery leftJoinCommonBlocFichierEnveloppe($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonBlocFichierEnveloppe relation
 * @method CommonFichierEnveloppeQuery rightJoinCommonBlocFichierEnveloppe($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonBlocFichierEnveloppe relation
 * @method CommonFichierEnveloppeQuery innerJoinCommonBlocFichierEnveloppe($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonBlocFichierEnveloppe relation
 *
...
abstract class BaseCommonFichierEnveloppeQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related CommonBlocFichierEnveloppe object
     *
     * @param   CommonBlocFichierEnveloppe|PropelObjectCollection $commonBlocFichierEnveloppe  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonFichierEnveloppeQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonBlocFichierEnveloppe($commonBlocFichierEnveloppe, $comparison = null)
    {
        if ($commonBlocFichierEnveloppe instanceof CommonBlocFichierEnveloppe) {
            return $this
                ->addUsingAlias(CommonFichierEnveloppePeer::ID_FICHIER, $commonBlocFichierEnveloppe->getIdFichier(), $comparison)
                ->addUsingAlias(CommonFichierEnveloppePeer::ORGANISME, $commonBlocFichierEnveloppe->getOrganisme(), $comparison);
        } else {
            throw new PropelException('filterByCommonBlocFichierEnveloppe() only accepts arguments of type CommonBlocFichierEnveloppe');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonBlocFichierEnveloppe relation
     *
     ...
     * @return CommonFichierEnveloppeQuery The current query, for fluid interface
     */
    public function joinCommonBlocFichierEnveloppe($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonBlocFichierEnveloppe');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonBlocFichierEnveloppe');
        }
    ...
    /**
     * Use the CommonBlocFichierEnveloppe relation CommonBlocFichierEnveloppe object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonBlocFichierEnveloppeQuery A secondary query class using the current class as primary query
     */
    public function useCommonBlocFichierEnveloppeQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonBlocFichierEnveloppe($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonBlocFichierEnveloppe', 'CommonBlocFichierEnveloppeQuery');
    }
