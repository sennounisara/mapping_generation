...
abstract class BaseCommonFichierEnveloppe extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonBlocFichierEnveloppe[] Collection to store aggregation of CommonBlocFichierEnveloppe objects.
     */
    protected $collCommonBlocFichierEnveloppes;
    protected $collCommonBlocFichierEnveloppesPartial;
     ...
     */
    protected $commonBlocFichierEnveloppesScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collCommonBlocFichierEnveloppes = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonBlocFichierEnveloppesScheduledForDeletion !== null) {
                if (!$this->commonBlocFichierEnveloppesScheduledForDeletion->isEmpty()) {
                    foreach ($this->commonBlocFichierEnveloppesScheduledForDeletion as $commonBlocFichierEnveloppe) {
                        // need to save related object because we set the relation to null
                        $commonBlocFichierEnveloppe->save($con);
                    }
                    $this->commonBlocFichierEnveloppesScheduledForDeletion = null;
                }
            }

            if ($this->collCommonBlocFichierEnveloppes !== null) {
                foreach ($this->collCommonBlocFichierEnveloppes as $referrerFK) {
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collCommonBlocFichierEnveloppes) {
                $result['CommonBlocFichierEnveloppes'] = $this->collCommonBlocFichierEnveloppes->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonBlocFichierEnveloppes() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonBlocFichierEnveloppe($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('CommonBlocFichierEnveloppe' == $relationName) {
            $this->initCommonBlocFichierEnveloppes();
        }
    }

    /**
     * Clears out the collCommonBlocFichierEnveloppes collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonFichierEnveloppe The current object (for fluent API support)
     * @see        addCommonBlocFichierEnveloppes()
     */
    public function clearCommonBlocFichierEnveloppes()
    {
        $this->collCommonBlocFichierEnveloppes = null; // important to set this to null since that means it is uninitialized
        $this->collCommonBlocFichierEnveloppesPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonBlocFichierEnveloppes collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonBlocFichierEnveloppes($v = true)
    {
        $this->collCommonBlocFichierEnveloppesPartial = $v;
    }

    /**
     * Initializes the collCommonBlocFichierEnveloppes collection.
     *
     * By default this just sets the collCommonBlocFichierEnveloppes collection to an empty array (like clearcollCommonBlocFichierEnveloppes());
     ...
     * @return void
     */
    public function initCommonBlocFichierEnveloppes($overrideExisting = true)
    {
        if (null !== $this->collCommonBlocFichierEnveloppes && !$overrideExisting) {
            return;
        }
        $this->collCommonBlocFichierEnveloppes = new PropelObjectCollection();
        $this->collCommonBlocFichierEnveloppes->setModel('CommonBlocFichierEnveloppe');
    }

    /**
     * Gets an array of CommonBlocFichierEnveloppe objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this CommonFichierEnveloppe is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonBlocFichierEnveloppe[] List of CommonBlocFichierEnveloppe objects
     * @throws PropelException
     */
    public function getCommonBlocFichierEnveloppes($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonBlocFichierEnveloppesPartial && !$this->isNew();
        if (null === $this->collCommonBlocFichierEnveloppes || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonBlocFichierEnveloppes) {
                // return empty collection
                $this->initCommonBlocFichierEnveloppes();
            } else {
                $collCommonBlocFichierEnveloppes = CommonBlocFichierEnveloppeQuery::create(null, $criteria)
                    ->filterByCommonFichierEnveloppe($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonBlocFichierEnveloppesPartial && count($collCommonBlocFichierEnveloppes)) {
                      $this->initCommonBlocFichierEnveloppes(false);

                      foreach ($collCommonBlocFichierEnveloppes as $obj) {
                        if (false == $this->collCommonBlocFichierEnveloppes->contains($obj)) {
                          $this->collCommonBlocFichierEnveloppes->append($obj);
                        }
                      }

                      $this->collCommonBlocFichierEnveloppesPartial = true;
                    }

                    $collCommonBlocFichierEnveloppes->getInternalIterator()->rewind();

                    return $collCommonBlocFichierEnveloppes;
                }

                if ($partial && $this->collCommonBlocFichierEnveloppes) {
                    foreach ($this->collCommonBlocFichierEnveloppes as $obj) {
                        if ($obj->isNew()) {
                            $collCommonBlocFichierEnveloppes[] = $obj;
                        }
                    }
                }

                $this->collCommonBlocFichierEnveloppes = $collCommonBlocFichierEnveloppes;
                $this->collCommonBlocFichierEnveloppesPartial = false;
            }
        }

        return $this->collCommonBlocFichierEnveloppes;
    }

    /**
     * Sets a collection of CommonBlocFichierEnveloppe objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonBlocFichierEnveloppes A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonFichierEnveloppe The current object (for fluent API support)
     */
    public function setCommonBlocFichierEnveloppes(PropelCollection $commonBlocFichierEnveloppes, PropelPDO $con = null)
    {
        $commonBlocFichierEnveloppesToDelete = $this->getCommonBlocFichierEnveloppes(new Criteria(), $con)->diff($commonBlocFichierEnveloppes);


        //since at least one column in the foreign key is at the same time a PK
        //we can not just set a PK to NULL in the lines below. We have to store
        //a backup of all values, so we are able to manipulate these items based on the onDelete value later.
        $this->commonBlocFichierEnveloppesScheduledForDeletion = clone $commonBlocFichierEnveloppesToDelete;

        foreach ($commonBlocFichierEnveloppesToDelete as $commonBlocFichierEnveloppeRemoved) {
            $commonBlocFichierEnveloppeRemoved->setCommonFichierEnveloppe(null);
        }

        $this->collCommonBlocFichierEnveloppes = null;
        foreach ($commonBlocFichierEnveloppes as $commonBlocFichierEnveloppe) {
            $this->addCommonBlocFichierEnveloppe($commonBlocFichierEnveloppe);
        }

        $this->collCommonBlocFichierEnveloppes = $commonBlocFichierEnveloppes;
        $this->collCommonBlocFichierEnveloppesPartial = false;
    ...
    /**
     * Returns the number of related CommonBlocFichierEnveloppe objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonBlocFichierEnveloppe objects.
     * @throws PropelException
     */
    public function countCommonBlocFichierEnveloppes(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonBlocFichierEnveloppesPartial && !$this->isNew();
        if (null === $this->collCommonBlocFichierEnveloppes || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonBlocFichierEnveloppes) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonBlocFichierEnveloppes());
            }
            $query = CommonBlocFichierEnveloppeQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonBlocFichierEnveloppes);
    }

    /**
     * Method called to associate a CommonBlocFichierEnveloppe object to this object
     * through the CommonBlocFichierEnveloppe foreign key attribute.
     *
     * @param   CommonBlocFichierEnveloppe $l CommonBlocFichierEnveloppe
     * @return CommonFichierEnveloppe The current object (for fluent API support)
     */
    public function addCommonBlocFichierEnveloppe(CommonBlocFichierEnveloppe $l)
    {
        if ($this->collCommonBlocFichierEnveloppes === null) {
            $this->initCommonBlocFichierEnveloppes();
            $this->collCommonBlocFichierEnveloppesPartial = true;
        }
        if (!in_array($l, $this->collCommonBlocFichierEnveloppes->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonBlocFichierEnveloppe($l);
        }
    ...
    /**
     * @param	CommonBlocFichierEnveloppe $commonBlocFichierEnveloppe The commonBlocFichierEnveloppe object to add.
     */
    protected function doAddCommonBlocFichierEnveloppe($commonBlocFichierEnveloppe)
    {
        $this->collCommonBlocFichierEnveloppes[]= $commonBlocFichierEnveloppe;
        $commonBlocFichierEnveloppe->setCommonFichierEnveloppe($this);
    }

    /**
     * @param	CommonBlocFichierEnveloppe $commonBlocFichierEnveloppe The commonBlocFichierEnveloppe object to remove.
     * @return CommonFichierEnveloppe The current object (for fluent API support)
     */
    public function removeCommonBlocFichierEnveloppe($commonBlocFichierEnveloppe)
    {
        if ($this->getCommonBlocFichierEnveloppes()->contains($commonBlocFichierEnveloppe)) {
            $this->collCommonBlocFichierEnveloppes->remove($this->collCommonBlocFichierEnveloppes->search($commonBlocFichierEnveloppe));
            if (null === $this->commonBlocFichierEnveloppesScheduledForDeletion) {
                $this->commonBlocFichierEnveloppesScheduledForDeletion = clone $this->collCommonBlocFichierEnveloppes;
                $this->commonBlocFichierEnveloppesScheduledForDeletion->clear();
            }
            $this->commonBlocFichierEnveloppesScheduledForDeletion[]= clone $commonBlocFichierEnveloppe;
            $commonBlocFichierEnveloppe->setCommonFichierEnveloppe(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonBlocFichierEnveloppes) {
                foreach ($this->collCommonBlocFichierEnveloppes as $o) {
        ...
        if ($this->collCommonBlocFichierEnveloppes instanceof PropelCollection) {
            $this->collCommonBlocFichierEnveloppes->clearIterator();
        }
        $this->collCommonBlocFichierEnveloppes = null;
    }
