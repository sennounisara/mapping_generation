 ...
 *
 * @method CommonEnveloppeItemCritereEvaluationQuery leftJoinCommonEnveloppeCritereEvaluation($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonEnveloppeCritereEvaluation relation
 * @method CommonEnveloppeItemCritereEvaluationQuery rightJoinCommonEnveloppeCritereEvaluation($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonEnveloppeCritereEvaluation relation
 * @method CommonEnveloppeItemCritereEvaluationQuery innerJoinCommonEnveloppeCritereEvaluation($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonEnveloppeCritereEvaluation relation
 *
...
abstract class BaseCommonEnveloppeItemCritereEvaluationQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @see       filterByCommonEnveloppeCritereEvaluation()
     *
    ...
    /**
     * Filter the query by a related CommonEnveloppeCritereEvaluation object
     *
     * @param   CommonEnveloppeCritereEvaluation|PropelObjectCollection $commonEnveloppeCritereEvaluation The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonEnveloppeItemCritereEvaluationQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonEnveloppeCritereEvaluation($commonEnveloppeCritereEvaluation, $comparison = null)
    {
        if ($commonEnveloppeCritereEvaluation instanceof CommonEnveloppeCritereEvaluation) {
            return $this
                ->addUsingAlias(CommonEnveloppeItemCritereEvaluationPeer::IDENVELOPPECRITEREEVALUATION, $commonEnveloppeCritereEvaluation->getId(), $comparison);
        } elseif ($commonEnveloppeCritereEvaluation instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(CommonEnveloppeItemCritereEvaluationPeer::IDENVELOPPECRITEREEVALUATION, $commonEnveloppeCritereEvaluation->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByCommonEnveloppeCritereEvaluation() only accepts arguments of type CommonEnveloppeCritereEvaluation or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonEnveloppeCritereEvaluation relation
     *
     ...
     * @return CommonEnveloppeItemCritereEvaluationQuery The current query, for fluid interface
     */
    public function joinCommonEnveloppeCritereEvaluation($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonEnveloppeCritereEvaluation');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonEnveloppeCritereEvaluation');
        }
    ...
    /**
     * Use the CommonEnveloppeCritereEvaluation relation CommonEnveloppeCritereEvaluation object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonEnveloppeCritereEvaluationQuery A secondary query class using the current class as primary query
     */
    public function useCommonEnveloppeCritereEvaluationQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonEnveloppeCritereEvaluation($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonEnveloppeCritereEvaluation', 'CommonEnveloppeCritereEvaluationQuery');
    }
