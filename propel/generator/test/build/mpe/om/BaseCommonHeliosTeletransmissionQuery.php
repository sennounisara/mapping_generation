 ...
 *
 * @method CommonHeliosTeletransmissionQuery leftJoinCommonHeliosTeletransmissionLot($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonHeliosTeletransmissionLot relation
 * @method CommonHeliosTeletransmissionQuery rightJoinCommonHeliosTeletransmissionLot($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonHeliosTeletransmissionLot relation
 * @method CommonHeliosTeletransmissionQuery innerJoinCommonHeliosTeletransmissionLot($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonHeliosTeletransmissionLot relation
 *
...
abstract class BaseCommonHeliosTeletransmissionQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related CommonHeliosTeletransmissionLot object
     *
     * @param   CommonHeliosTeletransmissionLot|PropelObjectCollection $commonHeliosTeletransmissionLot  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonHeliosTeletransmissionQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonHeliosTeletransmissionLot($commonHeliosTeletransmissionLot, $comparison = null)
    {
        if ($commonHeliosTeletransmissionLot instanceof CommonHeliosTeletransmissionLot) {
            return $this
                ->addUsingAlias(CommonHeliosTeletransmissionPeer::ID, $commonHeliosTeletransmissionLot->getIdTeletransmission(), $comparison)
                ->addUsingAlias(CommonHeliosTeletransmissionPeer::ORGANISME, $commonHeliosTeletransmissionLot->getOrganisme(), $comparison);
        } else {
            throw new PropelException('filterByCommonHeliosTeletransmissionLot() only accepts arguments of type CommonHeliosTeletransmissionLot');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonHeliosTeletransmissionLot relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return CommonHeliosTeletransmissionQuery The current query, for fluid interface
     */
    public function joinCommonHeliosTeletransmissionLot($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonHeliosTeletransmissionLot');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonHeliosTeletransmissionLot');
        }
    ...
    /**
     * Use the CommonHeliosTeletransmissionLot relation CommonHeliosTeletransmissionLot object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonHeliosTeletransmissionLotQuery A secondary query class using the current class as primary query
     */
    public function useCommonHeliosTeletransmissionLotQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonHeliosTeletransmissionLot($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonHeliosTeletransmissionLot', 'CommonHeliosTeletransmissionLotQuery');
    }
