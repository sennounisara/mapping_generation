 ...
 *
 * @method CommonItemFormulaireConsultationQuery leftJoinCommonConsultationFormulaire($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonConsultationFormulaire relation
 * @method CommonItemFormulaireConsultationQuery rightJoinCommonConsultationFormulaire($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonConsultationFormulaire relation
 * @method CommonItemFormulaireConsultationQuery innerJoinCommonConsultationFormulaire($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonConsultationFormulaire relation
 *
 * @method CommonItemFormulaireConsultationQuery leftJoinCommonEnveloppeItemFormulaireConsultationValues($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonEnveloppeItemFormulaireConsultationValues relation
 * @method CommonItemFormulaireConsultationQuery rightJoinCommonEnveloppeItemFormulaireConsultationValues($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonEnveloppeItemFormulaireConsultationValues relation
 * @method CommonItemFormulaireConsultationQuery innerJoinCommonEnveloppeItemFormulaireConsultationValues($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonEnveloppeItemFormulaireConsultationValues relation
 *
 * @method CommonItemFormulaireConsultationQuery leftJoinCommonReponseInscritItemFormulaireConsultationValues($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonReponseInscritItemFormulaireConsultationValues relation
 * @method CommonItemFormulaireConsultationQuery rightJoinCommonReponseInscritItemFormulaireConsultationValues($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonReponseInscritItemFormulaireConsultationValues relation
 * @method CommonItemFormulaireConsultationQuery innerJoinCommonReponseInscritItemFormulaireConsultationValues($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonReponseInscritItemFormulaireConsultationValues relation
 *
...
abstract class BaseCommonItemFormulaireConsultationQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @see       filterByCommonConsultationFormulaire()
     *
    ...
    /**
     * Filter the query by a related CommonConsultationFormulaire object
     *
     * @param   CommonConsultationFormulaire|PropelObjectCollection $commonConsultationFormulaire The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonItemFormulaireConsultationQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonConsultationFormulaire($commonConsultationFormulaire, $comparison = null)
    {
        if ($commonConsultationFormulaire instanceof CommonConsultationFormulaire) {
            return $this
                ->addUsingAlias(CommonItemFormulaireConsultationPeer::IDFORMULAIRECONSULTATION, $commonConsultationFormulaire->getId(), $comparison);
        } elseif ($commonConsultationFormulaire instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(CommonItemFormulaireConsultationPeer::IDFORMULAIRECONSULTATION, $commonConsultationFormulaire->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByCommonConsultationFormulaire() only accepts arguments of type CommonConsultationFormulaire or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonConsultationFormulaire relation
     *
     ...
     * @return CommonItemFormulaireConsultationQuery The current query, for fluid interface
     */
    public function joinCommonConsultationFormulaire($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonConsultationFormulaire');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonConsultationFormulaire');
        }
    ...
    /**
     * Use the CommonConsultationFormulaire relation CommonConsultationFormulaire object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonConsultationFormulaireQuery A secondary query class using the current class as primary query
     */
    public function useCommonConsultationFormulaireQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonConsultationFormulaire($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonConsultationFormulaire', 'CommonConsultationFormulaireQuery');
    }

    /**
     * Filter the query by a related CommonEnveloppeItemFormulaireConsultationValues object
     *
     * @param   CommonEnveloppeItemFormulaireConsultationValues|PropelObjectCollection $commonEnveloppeItemFormulaireConsultationValues  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonItemFormulaireConsultationQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonEnveloppeItemFormulaireConsultationValues($commonEnveloppeItemFormulaireConsultationValues, $comparison = null)
    {
        if ($commonEnveloppeItemFormulaireConsultationValues instanceof CommonEnveloppeItemFormulaireConsultationValues) {
            return $this
                ->addUsingAlias(CommonItemFormulaireConsultationPeer::ID, $commonEnveloppeItemFormulaireConsultationValues->getIditemformulaireconsultation(), $comparison);
        } elseif ($commonEnveloppeItemFormulaireConsultationValues instanceof PropelObjectCollection) {
            return $this
                ->useCommonEnveloppeItemFormulaireConsultationValuesQuery()
                ->filterByPrimaryKeys($commonEnveloppeItemFormulaireConsultationValues->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByCommonEnveloppeItemFormulaireConsultationValues() only accepts arguments of type CommonEnveloppeItemFormulaireConsultationValues or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonEnveloppeItemFormulaireConsultationValues relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return CommonItemFormulaireConsultationQuery The current query, for fluid interface
     */
    public function joinCommonEnveloppeItemFormulaireConsultationValues($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonEnveloppeItemFormulaireConsultationValues');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonEnveloppeItemFormulaireConsultationValues');
        }
    ...
    /**
     * Use the CommonEnveloppeItemFormulaireConsultationValues relation CommonEnveloppeItemFormulaireConsultationValues object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonEnveloppeItemFormulaireConsultationValuesQuery A secondary query class using the current class as primary query
     */
    public function useCommonEnveloppeItemFormulaireConsultationValuesQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonEnveloppeItemFormulaireConsultationValues($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonEnveloppeItemFormulaireConsultationValues', 'CommonEnveloppeItemFormulaireConsultationValuesQuery');
    }

    /**
     * Filter the query by a related CommonReponseInscritItemFormulaireConsultationValues object
     *
     * @param   CommonReponseInscritItemFormulaireConsultationValues|PropelObjectCollection $commonReponseInscritItemFormulaireConsultationValues  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonItemFormulaireConsultationQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonReponseInscritItemFormulaireConsultationValues($commonReponseInscritItemFormulaireConsultationValues, $comparison = null)
    {
        if ($commonReponseInscritItemFormulaireConsultationValues instanceof CommonReponseInscritItemFormulaireConsultationValues) {
            return $this
                ->addUsingAlias(CommonItemFormulaireConsultationPeer::ID, $commonReponseInscritItemFormulaireConsultationValues->getIditemformulaireconsultation(), $comparison);
        } elseif ($commonReponseInscritItemFormulaireConsultationValues instanceof PropelObjectCollection) {
            return $this
                ->useCommonReponseInscritItemFormulaireConsultationValuesQuery()
                ->filterByPrimaryKeys($commonReponseInscritItemFormulaireConsultationValues->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByCommonReponseInscritItemFormulaireConsultationValues() only accepts arguments of type CommonReponseInscritItemFormulaireConsultationValues or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonReponseInscritItemFormulaireConsultationValues relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return CommonItemFormulaireConsultationQuery The current query, for fluid interface
     */
    public function joinCommonReponseInscritItemFormulaireConsultationValues($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonReponseInscritItemFormulaireConsultationValues');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonReponseInscritItemFormulaireConsultationValues');
        }
    ...
    /**
     * Use the CommonReponseInscritItemFormulaireConsultationValues relation CommonReponseInscritItemFormulaireConsultationValues object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonReponseInscritItemFormulaireConsultationValuesQuery A secondary query class using the current class as primary query
     */
    public function useCommonReponseInscritItemFormulaireConsultationValuesQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonReponseInscritItemFormulaireConsultationValues($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonReponseInscritItemFormulaireConsultationValues', 'CommonReponseInscritItemFormulaireConsultationValuesQuery');
    }
