...
abstract class BaseCommonNewsletter extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonNewsletterPieceJointe[] Collection to store aggregation of CommonNewsletterPieceJointe objects.
     */
    protected $collCommonNewsletterPieceJointes;
    protected $collCommonNewsletterPieceJointesPartial;
     ...
     */
    protected $commonNewsletterPieceJointesScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collCommonNewsletterPieceJointes = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonNewsletterPieceJointesScheduledForDeletion !== null) {
                if (!$this->commonNewsletterPieceJointesScheduledForDeletion->isEmpty()) {
                    //the foreign key is flagged as `CASCADE`, so we delete the items
                    CommonNewsletterPieceJointeQuery::create()
                        ->filterByPrimaryKeys($this->commonNewsletterPieceJointesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->commonNewsletterPieceJointesScheduledForDeletion = null;
                }
            }

            if ($this->collCommonNewsletterPieceJointes !== null) {
                foreach ($this->collCommonNewsletterPieceJointes as $referrerFK) {
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collCommonNewsletterPieceJointes) {
                $result['CommonNewsletterPieceJointes'] = $this->collCommonNewsletterPieceJointes->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonNewsletterPieceJointes() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonNewsletterPieceJointe($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('CommonNewsletterPieceJointe' == $relationName) {
            $this->initCommonNewsletterPieceJointes();
        }
    }

    /**
     * Clears out the collCommonNewsletterPieceJointes collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonNewsletter The current object (for fluent API support)
     * @see        addCommonNewsletterPieceJointes()
     */
    public function clearCommonNewsletterPieceJointes()
    {
        $this->collCommonNewsletterPieceJointes = null; // important to set this to null since that means it is uninitialized
        $this->collCommonNewsletterPieceJointesPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonNewsletterPieceJointes collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonNewsletterPieceJointes($v = true)
    {
        $this->collCommonNewsletterPieceJointesPartial = $v;
    }

    /**
     * Initializes the collCommonNewsletterPieceJointes collection.
     *
     * By default this just sets the collCommonNewsletterPieceJointes collection to an empty array (like clearcollCommonNewsletterPieceJointes());
     ...
     * @return void
     */
    public function initCommonNewsletterPieceJointes($overrideExisting = true)
    {
        if (null !== $this->collCommonNewsletterPieceJointes && !$overrideExisting) {
            return;
        }
        $this->collCommonNewsletterPieceJointes = new PropelObjectCollection();
        $this->collCommonNewsletterPieceJointes->setModel('CommonNewsletterPieceJointe');
    }

    /**
     * Gets an array of CommonNewsletterPieceJointe objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this CommonNewsletter is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonNewsletterPieceJointe[] List of CommonNewsletterPieceJointe objects
     * @throws PropelException
     */
    public function getCommonNewsletterPieceJointes($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonNewsletterPieceJointesPartial && !$this->isNew();
        if (null === $this->collCommonNewsletterPieceJointes || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonNewsletterPieceJointes) {
                // return empty collection
                $this->initCommonNewsletterPieceJointes();
            } else {
                $collCommonNewsletterPieceJointes = CommonNewsletterPieceJointeQuery::create(null, $criteria)
                    ->filterByCommonNewsletter($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonNewsletterPieceJointesPartial && count($collCommonNewsletterPieceJointes)) {
                      $this->initCommonNewsletterPieceJointes(false);

                      foreach ($collCommonNewsletterPieceJointes as $obj) {
                        if (false == $this->collCommonNewsletterPieceJointes->contains($obj)) {
                          $this->collCommonNewsletterPieceJointes->append($obj);
                        }
                      }

                      $this->collCommonNewsletterPieceJointesPartial = true;
                    }

                    $collCommonNewsletterPieceJointes->getInternalIterator()->rewind();

                    return $collCommonNewsletterPieceJointes;
                }

                if ($partial && $this->collCommonNewsletterPieceJointes) {
                    foreach ($this->collCommonNewsletterPieceJointes as $obj) {
                        if ($obj->isNew()) {
                            $collCommonNewsletterPieceJointes[] = $obj;
                        }
                    }
                }

                $this->collCommonNewsletterPieceJointes = $collCommonNewsletterPieceJointes;
                $this->collCommonNewsletterPieceJointesPartial = false;
            }
        }

        return $this->collCommonNewsletterPieceJointes;
    }

    /**
     * Sets a collection of CommonNewsletterPieceJointe objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonNewsletterPieceJointes A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonNewsletter The current object (for fluent API support)
     */
    public function setCommonNewsletterPieceJointes(PropelCollection $commonNewsletterPieceJointes, PropelPDO $con = null)
    {
        $commonNewsletterPieceJointesToDelete = $this->getCommonNewsletterPieceJointes(new Criteria(), $con)->diff($commonNewsletterPieceJointes);


        $this->commonNewsletterPieceJointesScheduledForDeletion = $commonNewsletterPieceJointesToDelete;

        foreach ($commonNewsletterPieceJointesToDelete as $commonNewsletterPieceJointeRemoved) {
            $commonNewsletterPieceJointeRemoved->setCommonNewsletter(null);
        }

        $this->collCommonNewsletterPieceJointes = null;
        foreach ($commonNewsletterPieceJointes as $commonNewsletterPieceJointe) {
            $this->addCommonNewsletterPieceJointe($commonNewsletterPieceJointe);
        }

        $this->collCommonNewsletterPieceJointes = $commonNewsletterPieceJointes;
        $this->collCommonNewsletterPieceJointesPartial = false;
    ...
    /**
     * Returns the number of related CommonNewsletterPieceJointe objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonNewsletterPieceJointe objects.
     * @throws PropelException
     */
    public function countCommonNewsletterPieceJointes(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonNewsletterPieceJointesPartial && !$this->isNew();
        if (null === $this->collCommonNewsletterPieceJointes || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonNewsletterPieceJointes) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonNewsletterPieceJointes());
            }
            $query = CommonNewsletterPieceJointeQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonNewsletterPieceJointes);
    }

    /**
     * Method called to associate a CommonNewsletterPieceJointe object to this object
     * through the CommonNewsletterPieceJointe foreign key attribute.
     *
     * @param   CommonNewsletterPieceJointe $l CommonNewsletterPieceJointe
     * @return CommonNewsletter The current object (for fluent API support)
     */
    public function addCommonNewsletterPieceJointe(CommonNewsletterPieceJointe $l)
    {
        if ($this->collCommonNewsletterPieceJointes === null) {
            $this->initCommonNewsletterPieceJointes();
            $this->collCommonNewsletterPieceJointesPartial = true;
        }
        if (!in_array($l, $this->collCommonNewsletterPieceJointes->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonNewsletterPieceJointe($l);
        }
    ...
    /**
     * @param	CommonNewsletterPieceJointe $commonNewsletterPieceJointe The commonNewsletterPieceJointe object to add.
     */
    protected function doAddCommonNewsletterPieceJointe($commonNewsletterPieceJointe)
    {
        $this->collCommonNewsletterPieceJointes[]= $commonNewsletterPieceJointe;
        $commonNewsletterPieceJointe->setCommonNewsletter($this);
    }

    /**
     * @param	CommonNewsletterPieceJointe $commonNewsletterPieceJointe The commonNewsletterPieceJointe object to remove.
     * @return CommonNewsletter The current object (for fluent API support)
     */
    public function removeCommonNewsletterPieceJointe($commonNewsletterPieceJointe)
    {
        if ($this->getCommonNewsletterPieceJointes()->contains($commonNewsletterPieceJointe)) {
            $this->collCommonNewsletterPieceJointes->remove($this->collCommonNewsletterPieceJointes->search($commonNewsletterPieceJointe));
            if (null === $this->commonNewsletterPieceJointesScheduledForDeletion) {
                $this->commonNewsletterPieceJointesScheduledForDeletion = clone $this->collCommonNewsletterPieceJointes;
                $this->commonNewsletterPieceJointesScheduledForDeletion->clear();
            }
            $this->commonNewsletterPieceJointesScheduledForDeletion[]= clone $commonNewsletterPieceJointe;
            $commonNewsletterPieceJointe->setCommonNewsletter(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonNewsletterPieceJointes) {
                foreach ($this->collCommonNewsletterPieceJointes as $o) {
        ...
        if ($this->collCommonNewsletterPieceJointes instanceof PropelCollection) {
            $this->collCommonNewsletterPieceJointes->clearIterator();
        }
        $this->collCommonNewsletterPieceJointes = null;
    }
