 ...
 * @method CommonEntrepriseQuery orderByGuid($order = Criteria::ASC) Order by the guid column
 * @method CommonEntrepriseQuery orderByGeoId($order = Criteria::ASC) Order by the geo_id column
 *
 ...
 * @method CommonEntrepriseQuery groupByGuid() Group by the guid column
 * @method CommonEntrepriseQuery groupByGeoId() Group by the geo_id column
 *
 ...
 * @method CommonEntreprise findOneByGuid(string $guid) Return the first CommonEntreprise filtered by the guid column
 * @method CommonEntreprise findOneByGeoId(string $geo_id) Return the first CommonEntreprise filtered by the geo_id column
 *
 ...
 * @method array findByGuid(string $guid) Return CommonEntreprise objects filtered by the guid column
 * @method array findByGeoId(string $geo_id) Return CommonEntreprise objects filtered by the geo_id column
 *
...
abstract class BaseCommonEntrepriseQuery extends ModelCriteria
{
    ...
    public function filterByGuid($guid = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(CommonEntreprisePeer::GUID, $guid, $comparison);
    }

    /**
     * Filter the query on the geo_id column
     *
     * Example usage:
     * <code>
     * $query->filterByGeoId('fooValue');   // WHERE geo_id = 'fooValue'
     * $query->filterByGeoId('%fooValue%'); // WHERE geo_id LIKE '%fooValue%'
     * </code>
     *
     * @param     string $geoId The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return CommonEntrepriseQuery The current query, for fluid interface
     */
    public function filterByGeoId($geoId = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($geoId)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $geoId)) {
                $geoId = str_replace('*', '%', $geoId);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(CommonEntreprisePeer::GEO_ID, $geoId, $comparison);
    }
