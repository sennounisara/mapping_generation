 ...
 *
 * @method CommonDocumentExterneQuery leftJoinCommonBlobOrganisme($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonBlobOrganisme relation
 * @method CommonDocumentExterneQuery rightJoinCommonBlobOrganisme($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonBlobOrganisme relation
 * @method CommonDocumentExterneQuery innerJoinCommonBlobOrganisme($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonBlobOrganisme relation
 *
...
abstract class BaseCommonDocumentExterneQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @see       filterByCommonBlobOrganisme()
     *
    ...
    /**
     * Filter the query by a related CommonBlobOrganisme object
     *
     * @param   CommonBlobOrganisme|PropelObjectCollection $commonBlobOrganisme The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonDocumentExterneQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonBlobOrganisme($commonBlobOrganisme, $comparison = null)
    {
        if ($commonBlobOrganisme instanceof CommonBlobOrganisme) {
            return $this
                ->addUsingAlias(CommonDocumentExternePeer::IDBLOB, $commonBlobOrganisme->getId(), $comparison);
        } elseif ($commonBlobOrganisme instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(CommonDocumentExternePeer::IDBLOB, $commonBlobOrganisme->toKeyValue('Id', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByCommonBlobOrganisme() only accepts arguments of type CommonBlobOrganisme or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonBlobOrganisme relation
     *
     ...
     * @return CommonDocumentExterneQuery The current query, for fluid interface
     */
    public function joinCommonBlobOrganisme($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonBlobOrganisme');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonBlobOrganisme');
        }
    ...
    /**
     * Use the CommonBlobOrganisme relation CommonBlobOrganisme object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonBlobOrganismeQuery A secondary query class using the current class as primary query
     */
    public function useCommonBlobOrganismeQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinCommonBlobOrganisme($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonBlobOrganisme', 'CommonBlobOrganismeQuery');
    }
