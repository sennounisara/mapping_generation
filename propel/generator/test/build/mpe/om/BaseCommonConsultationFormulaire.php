...
abstract class BaseCommonConsultationFormulaire extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonItemFormulaireConsultation[] Collection to store aggregation of CommonItemFormulaireConsultation objects.
     */
    protected $collCommonItemFormulaireConsultations;
    protected $collCommonItemFormulaireConsultationsPartial;
     ...
     */
    protected $commonItemFormulaireConsultationsScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collCommonItemFormulaireConsultations = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonItemFormulaireConsultationsScheduledForDeletion !== null) {
                if (!$this->commonItemFormulaireConsultationsScheduledForDeletion->isEmpty()) {
                    //the foreign key is flagged as `CASCADE`, so we delete the items
                    CommonItemFormulaireConsultationQuery::create()
                        ->filterByPrimaryKeys($this->commonItemFormulaireConsultationsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->commonItemFormulaireConsultationsScheduledForDeletion = null;
                }
            }

            if ($this->collCommonItemFormulaireConsultations !== null) {
                foreach ($this->collCommonItemFormulaireConsultations as $referrerFK) {
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collCommonItemFormulaireConsultations) {
                $result['CommonItemFormulaireConsultations'] = $this->collCommonItemFormulaireConsultations->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonItemFormulaireConsultations() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonItemFormulaireConsultation($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('CommonItemFormulaireConsultation' == $relationName) {
            $this->initCommonItemFormulaireConsultations();
        }
    }

    /**
     * Clears out the collCommonItemFormulaireConsultations collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonConsultationFormulaire The current object (for fluent API support)
     * @see        addCommonItemFormulaireConsultations()
     */
    public function clearCommonItemFormulaireConsultations()
    {
        $this->collCommonItemFormulaireConsultations = null; // important to set this to null since that means it is uninitialized
        $this->collCommonItemFormulaireConsultationsPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonItemFormulaireConsultations collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonItemFormulaireConsultations($v = true)
    {
        $this->collCommonItemFormulaireConsultationsPartial = $v;
    }

    /**
     * Initializes the collCommonItemFormulaireConsultations collection.
     *
     * By default this just sets the collCommonItemFormulaireConsultations collection to an empty array (like clearcollCommonItemFormulaireConsultations());
     ...
     * @return void
     */
    public function initCommonItemFormulaireConsultations($overrideExisting = true)
    {
        if (null !== $this->collCommonItemFormulaireConsultations && !$overrideExisting) {
            return;
        }
        $this->collCommonItemFormulaireConsultations = new PropelObjectCollection();
        $this->collCommonItemFormulaireConsultations->setModel('CommonItemFormulaireConsultation');
    }

    /**
     * Gets an array of CommonItemFormulaireConsultation objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this CommonConsultationFormulaire is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonItemFormulaireConsultation[] List of CommonItemFormulaireConsultation objects
     * @throws PropelException
     */
    public function getCommonItemFormulaireConsultations($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonItemFormulaireConsultationsPartial && !$this->isNew();
        if (null === $this->collCommonItemFormulaireConsultations || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonItemFormulaireConsultations) {
                // return empty collection
                $this->initCommonItemFormulaireConsultations();
            } else {
                $collCommonItemFormulaireConsultations = CommonItemFormulaireConsultationQuery::create(null, $criteria)
                    ->filterByCommonConsultationFormulaire($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonItemFormulaireConsultationsPartial && count($collCommonItemFormulaireConsultations)) {
                      $this->initCommonItemFormulaireConsultations(false);

                      foreach ($collCommonItemFormulaireConsultations as $obj) {
                        if (false == $this->collCommonItemFormulaireConsultations->contains($obj)) {
                          $this->collCommonItemFormulaireConsultations->append($obj);
                        }
                      }

                      $this->collCommonItemFormulaireConsultationsPartial = true;
                    }

                    $collCommonItemFormulaireConsultations->getInternalIterator()->rewind();

                    return $collCommonItemFormulaireConsultations;
                }

                if ($partial && $this->collCommonItemFormulaireConsultations) {
                    foreach ($this->collCommonItemFormulaireConsultations as $obj) {
                        if ($obj->isNew()) {
                            $collCommonItemFormulaireConsultations[] = $obj;
                        }
                    }
                }

                $this->collCommonItemFormulaireConsultations = $collCommonItemFormulaireConsultations;
                $this->collCommonItemFormulaireConsultationsPartial = false;
            }
        }

        return $this->collCommonItemFormulaireConsultations;
    }

    /**
     * Sets a collection of CommonItemFormulaireConsultation objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonItemFormulaireConsultations A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonConsultationFormulaire The current object (for fluent API support)
     */
    public function setCommonItemFormulaireConsultations(PropelCollection $commonItemFormulaireConsultations, PropelPDO $con = null)
    {
        $commonItemFormulaireConsultationsToDelete = $this->getCommonItemFormulaireConsultations(new Criteria(), $con)->diff($commonItemFormulaireConsultations);


        $this->commonItemFormulaireConsultationsScheduledForDeletion = $commonItemFormulaireConsultationsToDelete;

        foreach ($commonItemFormulaireConsultationsToDelete as $commonItemFormulaireConsultationRemoved) {
            $commonItemFormulaireConsultationRemoved->setCommonConsultationFormulaire(null);
        }

        $this->collCommonItemFormulaireConsultations = null;
        foreach ($commonItemFormulaireConsultations as $commonItemFormulaireConsultation) {
            $this->addCommonItemFormulaireConsultation($commonItemFormulaireConsultation);
        }

        $this->collCommonItemFormulaireConsultations = $commonItemFormulaireConsultations;
        $this->collCommonItemFormulaireConsultationsPartial = false;
    ...
    /**
     * Returns the number of related CommonItemFormulaireConsultation objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonItemFormulaireConsultation objects.
     * @throws PropelException
     */
    public function countCommonItemFormulaireConsultations(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonItemFormulaireConsultationsPartial && !$this->isNew();
        if (null === $this->collCommonItemFormulaireConsultations || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonItemFormulaireConsultations) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonItemFormulaireConsultations());
            }
            $query = CommonItemFormulaireConsultationQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonItemFormulaireConsultations);
    }

    /**
     * Method called to associate a CommonItemFormulaireConsultation object to this object
     * through the CommonItemFormulaireConsultation foreign key attribute.
     *
     * @param   CommonItemFormulaireConsultation $l CommonItemFormulaireConsultation
     * @return CommonConsultationFormulaire The current object (for fluent API support)
     */
    public function addCommonItemFormulaireConsultation(CommonItemFormulaireConsultation $l)
    {
        if ($this->collCommonItemFormulaireConsultations === null) {
            $this->initCommonItemFormulaireConsultations();
            $this->collCommonItemFormulaireConsultationsPartial = true;
        }
        if (!in_array($l, $this->collCommonItemFormulaireConsultations->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonItemFormulaireConsultation($l);
        }
    ...
    /**
     * @param	CommonItemFormulaireConsultation $commonItemFormulaireConsultation The commonItemFormulaireConsultation object to add.
     */
    protected function doAddCommonItemFormulaireConsultation($commonItemFormulaireConsultation)
    {
        $this->collCommonItemFormulaireConsultations[]= $commonItemFormulaireConsultation;
        $commonItemFormulaireConsultation->setCommonConsultationFormulaire($this);
    }

    /**
     * @param	CommonItemFormulaireConsultation $commonItemFormulaireConsultation The commonItemFormulaireConsultation object to remove.
     * @return CommonConsultationFormulaire The current object (for fluent API support)
     */
    public function removeCommonItemFormulaireConsultation($commonItemFormulaireConsultation)
    {
        if ($this->getCommonItemFormulaireConsultations()->contains($commonItemFormulaireConsultation)) {
            $this->collCommonItemFormulaireConsultations->remove($this->collCommonItemFormulaireConsultations->search($commonItemFormulaireConsultation));
            if (null === $this->commonItemFormulaireConsultationsScheduledForDeletion) {
                $this->commonItemFormulaireConsultationsScheduledForDeletion = clone $this->collCommonItemFormulaireConsultations;
                $this->commonItemFormulaireConsultationsScheduledForDeletion->clear();
            }
            $this->commonItemFormulaireConsultationsScheduledForDeletion[]= clone $commonItemFormulaireConsultation;
            $commonItemFormulaireConsultation->setCommonConsultationFormulaire(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonItemFormulaireConsultations) {
                foreach ($this->collCommonItemFormulaireConsultations as $o) {
        ...
        if ($this->collCommonItemFormulaireConsultations instanceof PropelCollection) {
            $this->collCommonItemFormulaireConsultations->clearIterator();
        }
        $this->collCommonItemFormulaireConsultations = null;
    }
