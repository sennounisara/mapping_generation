...
abstract class BaseCommonDecisionEnveloppe extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonChorusEchange[] Collection to store aggregation of CommonChorusEchange objects.
     */
    protected $collCommonChorusEchanges;
    protected $collCommonChorusEchangesPartial;
     ...
     */
    protected $commonChorusEchangesScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collCommonChorusEchanges = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonChorusEchangesScheduledForDeletion !== null) {
                if (!$this->commonChorusEchangesScheduledForDeletion->isEmpty()) {
                    //the foreign key is flagged as `CASCADE`, so we delete the items
                    CommonChorusEchangeQuery::create()
                        ->filterByPrimaryKeys($this->commonChorusEchangesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->commonChorusEchangesScheduledForDeletion = null;
                }
            }

            if ($this->collCommonChorusEchanges !== null) {
                foreach ($this->collCommonChorusEchanges as $referrerFK) {
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collCommonChorusEchanges) {
                $result['CommonChorusEchanges'] = $this->collCommonChorusEchanges->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonChorusEchanges() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonChorusEchange($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('CommonChorusEchange' == $relationName) {
            $this->initCommonChorusEchanges();
        }
    }

    /**
     * Clears out the collCommonChorusEchanges collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonDecisionEnveloppe The current object (for fluent API support)
     * @see        addCommonChorusEchanges()
     */
    public function clearCommonChorusEchanges()
    {
        $this->collCommonChorusEchanges = null; // important to set this to null since that means it is uninitialized
        $this->collCommonChorusEchangesPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonChorusEchanges collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonChorusEchanges($v = true)
    {
        $this->collCommonChorusEchangesPartial = $v;
    }

    /**
     * Initializes the collCommonChorusEchanges collection.
     *
     * By default this just sets the collCommonChorusEchanges collection to an empty array (like clearcollCommonChorusEchanges());
     ...
     * @return void
     */
    public function initCommonChorusEchanges($overrideExisting = true)
    {
        if (null !== $this->collCommonChorusEchanges && !$overrideExisting) {
            return;
        }
        $this->collCommonChorusEchanges = new PropelObjectCollection();
        $this->collCommonChorusEchanges->setModel('CommonChorusEchange');
    }

    /**
     * Gets an array of CommonChorusEchange objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this CommonDecisionEnveloppe is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonChorusEchange[] List of CommonChorusEchange objects
     * @throws PropelException
     */
    public function getCommonChorusEchanges($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonChorusEchangesPartial && !$this->isNew();
        if (null === $this->collCommonChorusEchanges || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonChorusEchanges) {
                // return empty collection
                $this->initCommonChorusEchanges();
            } else {
                $collCommonChorusEchanges = CommonChorusEchangeQuery::create(null, $criteria)
                    ->filterByCommonDecisionEnveloppe($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonChorusEchangesPartial && count($collCommonChorusEchanges)) {
                      $this->initCommonChorusEchanges(false);

                      foreach ($collCommonChorusEchanges as $obj) {
                        if (false == $this->collCommonChorusEchanges->contains($obj)) {
                          $this->collCommonChorusEchanges->append($obj);
                        }
                      }

                      $this->collCommonChorusEchangesPartial = true;
                    }

                    $collCommonChorusEchanges->getInternalIterator()->rewind();

                    return $collCommonChorusEchanges;
                }

                if ($partial && $this->collCommonChorusEchanges) {
                    foreach ($this->collCommonChorusEchanges as $obj) {
                        if ($obj->isNew()) {
                            $collCommonChorusEchanges[] = $obj;
                        }
                    }
                }

                $this->collCommonChorusEchanges = $collCommonChorusEchanges;
                $this->collCommonChorusEchangesPartial = false;
            }
        }

        return $this->collCommonChorusEchanges;
    }

    /**
     * Sets a collection of CommonChorusEchange objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonChorusEchanges A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonDecisionEnveloppe The current object (for fluent API support)
     */
    public function setCommonChorusEchanges(PropelCollection $commonChorusEchanges, PropelPDO $con = null)
    {
        $commonChorusEchangesToDelete = $this->getCommonChorusEchanges(new Criteria(), $con)->diff($commonChorusEchanges);


        //since at least one column in the foreign key is at the same time a PK
        //we can not just set a PK to NULL in the lines below. We have to store
        //a backup of all values, so we are able to manipulate these items based on the onDelete value later.
        $this->commonChorusEchangesScheduledForDeletion = clone $commonChorusEchangesToDelete;

        foreach ($commonChorusEchangesToDelete as $commonChorusEchangeRemoved) {
            $commonChorusEchangeRemoved->setCommonDecisionEnveloppe(null);
        }

        $this->collCommonChorusEchanges = null;
        foreach ($commonChorusEchanges as $commonChorusEchange) {
            $this->addCommonChorusEchange($commonChorusEchange);
        }

        $this->collCommonChorusEchanges = $commonChorusEchanges;
        $this->collCommonChorusEchangesPartial = false;
    ...
    /**
     * Returns the number of related CommonChorusEchange objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonChorusEchange objects.
     * @throws PropelException
     */
    public function countCommonChorusEchanges(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonChorusEchangesPartial && !$this->isNew();
        if (null === $this->collCommonChorusEchanges || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonChorusEchanges) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonChorusEchanges());
            }
            $query = CommonChorusEchangeQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonChorusEchanges);
    }

    /**
     * Method called to associate a CommonChorusEchange object to this object
     * through the CommonChorusEchange foreign key attribute.
     *
     * @param   CommonChorusEchange $l CommonChorusEchange
     * @return CommonDecisionEnveloppe The current object (for fluent API support)
     */
    public function addCommonChorusEchange(CommonChorusEchange $l)
    {
        if ($this->collCommonChorusEchanges === null) {
            $this->initCommonChorusEchanges();
            $this->collCommonChorusEchangesPartial = true;
        }
        if (!in_array($l, $this->collCommonChorusEchanges->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonChorusEchange($l);
        }
    ...
    /**
     * @param	CommonChorusEchange $commonChorusEchange The commonChorusEchange object to add.
     */
    protected function doAddCommonChorusEchange($commonChorusEchange)
    {
        $this->collCommonChorusEchanges[]= $commonChorusEchange;
        $commonChorusEchange->setCommonDecisionEnveloppe($this);
    }

    /**
     * @param	CommonChorusEchange $commonChorusEchange The commonChorusEchange object to remove.
     * @return CommonDecisionEnveloppe The current object (for fluent API support)
     */
    public function removeCommonChorusEchange($commonChorusEchange)
    {
        if ($this->getCommonChorusEchanges()->contains($commonChorusEchange)) {
            $this->collCommonChorusEchanges->remove($this->collCommonChorusEchanges->search($commonChorusEchange));
            if (null === $this->commonChorusEchangesScheduledForDeletion) {
                $this->commonChorusEchangesScheduledForDeletion = clone $this->collCommonChorusEchanges;
                $this->commonChorusEchangesScheduledForDeletion->clear();
            }
            $this->commonChorusEchangesScheduledForDeletion[]= clone $commonChorusEchange;
            $commonChorusEchange->setCommonDecisionEnveloppe(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonChorusEchanges) {
                foreach ($this->collCommonChorusEchanges as $o) {
        ...
        if ($this->collCommonChorusEchanges instanceof PropelCollection) {
            $this->collCommonChorusEchanges->clearIterator();
        }
        $this->collCommonChorusEchanges = null;
    }
