...
abstract class BaseCommonExecFacture extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonExecReception[] Collection to store aggregation of CommonExecReception objects.
     */
    protected $collCommonExecReceptions;
    protected $collCommonExecReceptionsPartial;
     ...
     * @var		PropelObjectCollection
     */
    protected $commonExecReceptionsScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collCommonExecReceptions = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonExecReceptionsScheduledForDeletion !== null) {
                if (!$this->commonExecReceptionsScheduledForDeletion->isEmpty()) {
                    foreach ($this->commonExecReceptionsScheduledForDeletion as $commonExecReception) {
                        // need to save related object because we set the relation to null
                        $commonExecReception->save($con);
                    }
                    $this->commonExecReceptionsScheduledForDeletion = null;
                }
            }

            if ($this->collCommonExecReceptions !== null) {
                foreach ($this->collCommonExecReceptions as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collCommonExecReceptions) {
                $result['CommonExecReceptions'] = $this->collCommonExecReceptions->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonExecReceptions() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonExecReception($relObj->copy($deepCopy));
                }
            }
    ...
    public function initRelation($relationName)
    {
        ...
        if ('CommonExecReception' == $relationName) {
            $this->initCommonExecReceptions();
        }
    ...
    /**
     * Clears out the collCommonExecReceptions collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonExecFacture The current object (for fluent API support)
     * @see        addCommonExecReceptions()
     */
    public function clearCommonExecReceptions()
    {
        $this->collCommonExecReceptions = null; // important to set this to null since that means it is uninitialized
        $this->collCommonExecReceptionsPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonExecReceptions collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonExecReceptions($v = true)
    {
        $this->collCommonExecReceptionsPartial = $v;
    }

    /**
     * Initializes the collCommonExecReceptions collection.
     *
     * By default this just sets the collCommonExecReceptions collection to an empty array (like clearcollCommonExecReceptions());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initCommonExecReceptions($overrideExisting = true)
    {
        if (null !== $this->collCommonExecReceptions && !$overrideExisting) {
            return;
        }
        $this->collCommonExecReceptions = new PropelObjectCollection();
        $this->collCommonExecReceptions->setModel('CommonExecReception');
    }

    /**
     * Gets an array of CommonExecReception objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonExecReception[] List of CommonExecReception objects
     * @throws PropelException
     */
    public function getCommonExecReceptions($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonExecReceptionsPartial && !$this->isNew();
        if (null === $this->collCommonExecReceptions || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonExecReceptions) {
                // return empty collection
                $this->initCommonExecReceptions();
            } else {
                $collCommonExecReceptions = CommonExecReceptionQuery::create(null, $criteria)
                    ->filterByCommonExecFacture($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonExecReceptionsPartial && count($collCommonExecReceptions)) {
                      $this->initCommonExecReceptions(false);

                      foreach ($collCommonExecReceptions as $obj) {
                        if (false == $this->collCommonExecReceptions->contains($obj)) {
                          $this->collCommonExecReceptions->append($obj);
                        }
                      }

                      $this->collCommonExecReceptionsPartial = true;
                    }

                    $collCommonExecReceptions->getInternalIterator()->rewind();

                    return $collCommonExecReceptions;
                }

                if ($partial && $this->collCommonExecReceptions) {
                    foreach ($this->collCommonExecReceptions as $obj) {
                        if ($obj->isNew()) {
                            $collCommonExecReceptions[] = $obj;
                        }
                    }
                }

                $this->collCommonExecReceptions = $collCommonExecReceptions;
                $this->collCommonExecReceptionsPartial = false;
            }
        }

        return $this->collCommonExecReceptions;
    }

    /**
     * Sets a collection of CommonExecReception objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonExecReceptions A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonExecFacture The current object (for fluent API support)
     */
    public function setCommonExecReceptions(PropelCollection $commonExecReceptions, PropelPDO $con = null)
    {
        $commonExecReceptionsToDelete = $this->getCommonExecReceptions(new Criteria(), $con)->diff($commonExecReceptions);


        $this->commonExecReceptionsScheduledForDeletion = $commonExecReceptionsToDelete;

        foreach ($commonExecReceptionsToDelete as $commonExecReceptionRemoved) {
            $commonExecReceptionRemoved->setCommonExecFacture(null);
        }

        $this->collCommonExecReceptions = null;
        foreach ($commonExecReceptions as $commonExecReception) {
            $this->addCommonExecReception($commonExecReception);
        }

        $this->collCommonExecReceptions = $commonExecReceptions;
        $this->collCommonExecReceptionsPartial = false;
    ...
    /**
     * Returns the number of related CommonExecReception objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonExecReception objects.
     * @throws PropelException
     */
    public function countCommonExecReceptions(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonExecReceptionsPartial && !$this->isNew();
        if (null === $this->collCommonExecReceptions || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonExecReceptions) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonExecReceptions());
            }
            $query = CommonExecReceptionQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonExecReceptions);
    }

    /**
     * Method called to associate a CommonExecReception object to this object
     * through the CommonExecReception foreign key attribute.
     *
     * @param   CommonExecReception $l CommonExecReception
     * @return CommonExecFacture The current object (for fluent API support)
     */
    public function addCommonExecReception(CommonExecReception $l)
    {
        if ($this->collCommonExecReceptions === null) {
            $this->initCommonExecReceptions();
            $this->collCommonExecReceptionsPartial = true;
        }
        if (!in_array($l, $this->collCommonExecReceptions->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonExecReception($l);
        }
    ...
    /**
     * @param	CommonExecReception $commonExecReception The commonExecReception object to add.
     */
    protected function doAddCommonExecReception($commonExecReception)
    {
        $this->collCommonExecReceptions[]= $commonExecReception;
        $commonExecReception->setCommonExecFacture($this);
    }

    /**
     * @param	CommonExecReception $commonExecReception The commonExecReception object to remove.
     * @return CommonExecFacture The current object (for fluent API support)
     */
    public function removeCommonExecReception($commonExecReception)
    {
        if ($this->getCommonExecReceptions()->contains($commonExecReception)) {
            $this->collCommonExecReceptions->remove($this->collCommonExecReceptions->search($commonExecReception));
            if (null === $this->commonExecReceptionsScheduledForDeletion) {
                $this->commonExecReceptionsScheduledForDeletion = clone $this->collCommonExecReceptions;
                $this->commonExecReceptionsScheduledForDeletion->clear();
            }
            $this->commonExecReceptionsScheduledForDeletion[]= clone $commonExecReception;
            $commonExecReception->setCommonExecFacture(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonExecReceptions) {
                foreach ($this->collCommonExecReceptions as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        ...
        $this->collCommonExecPieceFactures = null;
        if ($this->collCommonExecReceptions instanceof PropelCollection) {
            $this->collCommonExecReceptions->clearIterator();
        }
        $this->collCommonExecReceptions = null;
