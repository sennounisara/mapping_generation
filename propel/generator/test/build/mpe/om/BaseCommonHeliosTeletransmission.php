...
abstract class BaseCommonHeliosTeletransmission extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonHeliosTeletransmissionLot[] Collection to store aggregation of CommonHeliosTeletransmissionLot objects.
     */
    protected $collCommonHeliosTeletransmissionLots;
    protected $collCommonHeliosTeletransmissionLotsPartial;
     ...
     */
    protected $commonHeliosTeletransmissionLotsScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->aCommonConsultation = null;
            $this->collCommonHeliosTeletransmissionLots = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonHeliosTeletransmissionLotsScheduledForDeletion !== null) {
                if (!$this->commonHeliosTeletransmissionLotsScheduledForDeletion->isEmpty()) {
                    //the foreign key is flagged as `CASCADE`, so we delete the items
                    CommonHeliosTeletransmissionLotQuery::create()
                        ->filterByPrimaryKeys($this->commonHeliosTeletransmissionLotsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->commonHeliosTeletransmissionLotsScheduledForDeletion = null;
                }
            }

            if ($this->collCommonHeliosTeletransmissionLots !== null) {
                foreach ($this->collCommonHeliosTeletransmissionLots as $referrerFK) {
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collCommonHeliosTeletransmissionLots) {
                $result['CommonHeliosTeletransmissionLots'] = $this->collCommonHeliosTeletransmissionLots->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonHeliosTeletransmissionLots() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonHeliosTeletransmissionLot($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('CommonHeliosTeletransmissionLot' == $relationName) {
            $this->initCommonHeliosTeletransmissionLots();
        }
    }

    /**
     * Clears out the collCommonHeliosTeletransmissionLots collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonHeliosTeletransmission The current object (for fluent API support)
     * @see        addCommonHeliosTeletransmissionLots()
     */
    public function clearCommonHeliosTeletransmissionLots()
    {
        $this->collCommonHeliosTeletransmissionLots = null; // important to set this to null since that means it is uninitialized
        $this->collCommonHeliosTeletransmissionLotsPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonHeliosTeletransmissionLots collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonHeliosTeletransmissionLots($v = true)
    {
        $this->collCommonHeliosTeletransmissionLotsPartial = $v;
    }

    /**
     * Initializes the collCommonHeliosTeletransmissionLots collection.
     *
     * By default this just sets the collCommonHeliosTeletransmissionLots collection to an empty array (like clearcollCommonHeliosTeletransmissionLots());
     ...
     * @return void
     */
    public function initCommonHeliosTeletransmissionLots($overrideExisting = true)
    {
        if (null !== $this->collCommonHeliosTeletransmissionLots && !$overrideExisting) {
            return;
        }
        $this->collCommonHeliosTeletransmissionLots = new PropelObjectCollection();
        $this->collCommonHeliosTeletransmissionLots->setModel('CommonHeliosTeletransmissionLot');
    }

    /**
     * Gets an array of CommonHeliosTeletransmissionLot objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this CommonHeliosTeletransmission is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonHeliosTeletransmissionLot[] List of CommonHeliosTeletransmissionLot objects
     * @throws PropelException
     */
    public function getCommonHeliosTeletransmissionLots($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonHeliosTeletransmissionLotsPartial && !$this->isNew();
        if (null === $this->collCommonHeliosTeletransmissionLots || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonHeliosTeletransmissionLots) {
                // return empty collection
                $this->initCommonHeliosTeletransmissionLots();
            } else {
                $collCommonHeliosTeletransmissionLots = CommonHeliosTeletransmissionLotQuery::create(null, $criteria)
                    ->filterByCommonHeliosTeletransmission($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonHeliosTeletransmissionLotsPartial && count($collCommonHeliosTeletransmissionLots)) {
                      $this->initCommonHeliosTeletransmissionLots(false);

                      foreach ($collCommonHeliosTeletransmissionLots as $obj) {
                        if (false == $this->collCommonHeliosTeletransmissionLots->contains($obj)) {
                          $this->collCommonHeliosTeletransmissionLots->append($obj);
                        }
                      }

                      $this->collCommonHeliosTeletransmissionLotsPartial = true;
                    }

                    $collCommonHeliosTeletransmissionLots->getInternalIterator()->rewind();

                    return $collCommonHeliosTeletransmissionLots;
                }

                if ($partial && $this->collCommonHeliosTeletransmissionLots) {
                    foreach ($this->collCommonHeliosTeletransmissionLots as $obj) {
                        if ($obj->isNew()) {
                            $collCommonHeliosTeletransmissionLots[] = $obj;
                        }
                    }
                }

                $this->collCommonHeliosTeletransmissionLots = $collCommonHeliosTeletransmissionLots;
                $this->collCommonHeliosTeletransmissionLotsPartial = false;
            }
        }

        return $this->collCommonHeliosTeletransmissionLots;
    }

    /**
     * Sets a collection of CommonHeliosTeletransmissionLot objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonHeliosTeletransmissionLots A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonHeliosTeletransmission The current object (for fluent API support)
     */
    public function setCommonHeliosTeletransmissionLots(PropelCollection $commonHeliosTeletransmissionLots, PropelPDO $con = null)
    {
        $commonHeliosTeletransmissionLotsToDelete = $this->getCommonHeliosTeletransmissionLots(new Criteria(), $con)->diff($commonHeliosTeletransmissionLots);


        //since at least one column in the foreign key is at the same time a PK
        //we can not just set a PK to NULL in the lines below. We have to store
        //a backup of all values, so we are able to manipulate these items based on the onDelete value later.
        $this->commonHeliosTeletransmissionLotsScheduledForDeletion = clone $commonHeliosTeletransmissionLotsToDelete;

        foreach ($commonHeliosTeletransmissionLotsToDelete as $commonHeliosTeletransmissionLotRemoved) {
            $commonHeliosTeletransmissionLotRemoved->setCommonHeliosTeletransmission(null);
        }

        $this->collCommonHeliosTeletransmissionLots = null;
        foreach ($commonHeliosTeletransmissionLots as $commonHeliosTeletransmissionLot) {
            $this->addCommonHeliosTeletransmissionLot($commonHeliosTeletransmissionLot);
        }

        $this->collCommonHeliosTeletransmissionLots = $commonHeliosTeletransmissionLots;
        $this->collCommonHeliosTeletransmissionLotsPartial = false;
    ...
    /**
     * Returns the number of related CommonHeliosTeletransmissionLot objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonHeliosTeletransmissionLot objects.
     * @throws PropelException
     */
    public function countCommonHeliosTeletransmissionLots(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonHeliosTeletransmissionLotsPartial && !$this->isNew();
        if (null === $this->collCommonHeliosTeletransmissionLots || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonHeliosTeletransmissionLots) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonHeliosTeletransmissionLots());
            }
            $query = CommonHeliosTeletransmissionLotQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonHeliosTeletransmissionLots);
    }

    /**
     * Method called to associate a CommonHeliosTeletransmissionLot object to this object
     * through the CommonHeliosTeletransmissionLot foreign key attribute.
     *
     * @param   CommonHeliosTeletransmissionLot $l CommonHeliosTeletransmissionLot
     * @return CommonHeliosTeletransmission The current object (for fluent API support)
     */
    public function addCommonHeliosTeletransmissionLot(CommonHeliosTeletransmissionLot $l)
    {
        if ($this->collCommonHeliosTeletransmissionLots === null) {
            $this->initCommonHeliosTeletransmissionLots();
            $this->collCommonHeliosTeletransmissionLotsPartial = true;
        }
        if (!in_array($l, $this->collCommonHeliosTeletransmissionLots->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonHeliosTeletransmissionLot($l);
        }
    ...
    /**
     * @param	CommonHeliosTeletransmissionLot $commonHeliosTeletransmissionLot The commonHeliosTeletransmissionLot object to add.
     */
    protected function doAddCommonHeliosTeletransmissionLot($commonHeliosTeletransmissionLot)
    {
        $this->collCommonHeliosTeletransmissionLots[]= $commonHeliosTeletransmissionLot;
        $commonHeliosTeletransmissionLot->setCommonHeliosTeletransmission($this);
    }

    /**
     * @param	CommonHeliosTeletransmissionLot $commonHeliosTeletransmissionLot The commonHeliosTeletransmissionLot object to remove.
     * @return CommonHeliosTeletransmission The current object (for fluent API support)
     */
    public function removeCommonHeliosTeletransmissionLot($commonHeliosTeletransmissionLot)
    {
        if ($this->getCommonHeliosTeletransmissionLots()->contains($commonHeliosTeletransmissionLot)) {
            $this->collCommonHeliosTeletransmissionLots->remove($this->collCommonHeliosTeletransmissionLots->search($commonHeliosTeletransmissionLot));
            if (null === $this->commonHeliosTeletransmissionLotsScheduledForDeletion) {
                $this->commonHeliosTeletransmissionLotsScheduledForDeletion = clone $this->collCommonHeliosTeletransmissionLots;
                $this->commonHeliosTeletransmissionLotsScheduledForDeletion->clear();
            }
            $this->commonHeliosTeletransmissionLotsScheduledForDeletion[]= clone $commonHeliosTeletransmissionLot;
            $commonHeliosTeletransmissionLot->setCommonHeliosTeletransmission(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonHeliosTeletransmissionLots) {
                foreach ($this->collCommonHeliosTeletransmissionLots as $o) {
        ...
        if ($this->collCommonHeliosTeletransmissionLots instanceof PropelCollection) {
            $this->collCommonHeliosTeletransmissionLots->clearIterator();
        }
        $this->collCommonHeliosTeletransmissionLots = null;
        $this->aCommonConsultation = null;
    }
