 ...
 *
 * @method CommonAnnoncePressQuery leftJoinCommonAnnoncePressPieceJointe($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonAnnoncePressPieceJointe relation
 * @method CommonAnnoncePressQuery rightJoinCommonAnnoncePressPieceJointe($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonAnnoncePressPieceJointe relation
 * @method CommonAnnoncePressQuery innerJoinCommonAnnoncePressPieceJointe($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonAnnoncePressPieceJointe relation
 *
...
abstract class BaseCommonAnnoncePressQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related CommonAnnoncePressPieceJointe object
     *
     * @param   CommonAnnoncePressPieceJointe|PropelObjectCollection $commonAnnoncePressPieceJointe  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonAnnoncePressQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonAnnoncePressPieceJointe($commonAnnoncePressPieceJointe, $comparison = null)
    {
        if ($commonAnnoncePressPieceJointe instanceof CommonAnnoncePressPieceJointe) {
            return $this
                ->addUsingAlias(CommonAnnoncePressPeer::ID, $commonAnnoncePressPieceJointe->getIdAnnoncePress(), $comparison);
        } elseif ($commonAnnoncePressPieceJointe instanceof PropelObjectCollection) {
            return $this
                ->useCommonAnnoncePressPieceJointeQuery()
                ->filterByPrimaryKeys($commonAnnoncePressPieceJointe->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByCommonAnnoncePressPieceJointe() only accepts arguments of type CommonAnnoncePressPieceJointe or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonAnnoncePressPieceJointe relation
     *
     ...
     * @return CommonAnnoncePressQuery The current query, for fluid interface
     */
    public function joinCommonAnnoncePressPieceJointe($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonAnnoncePressPieceJointe');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonAnnoncePressPieceJointe');
        }
    ...
    /**
     * Use the CommonAnnoncePressPieceJointe relation CommonAnnoncePressPieceJointe object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonAnnoncePressPieceJointeQuery A secondary query class using the current class as primary query
     */
    public function useCommonAnnoncePressPieceJointeQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonAnnoncePressPieceJointe($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonAnnoncePressPieceJointe', 'CommonAnnoncePressPieceJointeQuery');
    }
