 ...
 *
 * @method CommonCentralePublicationQuery leftJoinCommonCompteCentrale($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonCompteCentrale relation
 * @method CommonCentralePublicationQuery rightJoinCommonCompteCentrale($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonCompteCentrale relation
 * @method CommonCentralePublicationQuery innerJoinCommonCompteCentrale($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonCompteCentrale relation
 *
...
abstract class BaseCommonCentralePublicationQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related CommonCompteCentrale object
     *
     * @param   CommonCompteCentrale|PropelObjectCollection $commonCompteCentrale  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonCentralePublicationQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonCompteCentrale($commonCompteCentrale, $comparison = null)
    {
        if ($commonCompteCentrale instanceof CommonCompteCentrale) {
            return $this
                ->addUsingAlias(CommonCentralePublicationPeer::ID, $commonCompteCentrale->getIdCentrale(), $comparison);
        } elseif ($commonCompteCentrale instanceof PropelObjectCollection) {
            return $this
                ->useCommonCompteCentraleQuery()
                ->filterByPrimaryKeys($commonCompteCentrale->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByCommonCompteCentrale() only accepts arguments of type CommonCompteCentrale or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonCompteCentrale relation
     *
     ...
     * @return CommonCentralePublicationQuery The current query, for fluid interface
     */
    public function joinCommonCompteCentrale($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonCompteCentrale');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonCompteCentrale');
        }
    ...
    /**
     * Use the CommonCompteCentrale relation CommonCompteCentrale object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonCompteCentraleQuery A secondary query class using the current class as primary query
     */
    public function useCommonCompteCentraleQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinCommonCompteCentrale($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonCompteCentrale', 'CommonCompteCentraleQuery');
    }
