 ...
 *
 * @method CommonBlocFichierEnveloppeQuery leftJoinCommonFichierEnveloppe($relationAlias = null) Adds a LEFT JOIN clause to the query using the CommonFichierEnveloppe relation
 * @method CommonBlocFichierEnveloppeQuery rightJoinCommonFichierEnveloppe($relationAlias = null) Adds a RIGHT JOIN clause to the query using the CommonFichierEnveloppe relation
 * @method CommonBlocFichierEnveloppeQuery innerJoinCommonFichierEnveloppe($relationAlias = null) Adds a INNER JOIN clause to the query using the CommonFichierEnveloppe relation
 *
...
abstract class BaseCommonBlocFichierEnveloppeQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @see       filterByCommonFichierEnveloppe()
     *
    ...
    /**
     * Filter the query by a related CommonFichierEnveloppe object
     *
     * @param   CommonFichierEnveloppe $commonFichierEnveloppe The related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 CommonBlocFichierEnveloppeQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
     */
    public function filterByCommonFichierEnveloppe($commonFichierEnveloppe, $comparison = null)
    {
        if ($commonFichierEnveloppe instanceof CommonFichierEnveloppe) {
            return $this
                ->addUsingAlias(CommonBlocFichierEnveloppePeer::ID_FICHIER, $commonFichierEnveloppe->getIdFichier(), $comparison)
                ->addUsingAlias(CommonBlocFichierEnveloppePeer::ORGANISME, $commonFichierEnveloppe->getOrganisme(), $comparison);
        } else {
            throw new PropelException('filterByCommonFichierEnveloppe() only accepts arguments of type CommonFichierEnveloppe');
        }
    }

    /**
     * Adds a JOIN clause to the query using the CommonFichierEnveloppe relation
     *
     ...
     * @return CommonBlocFichierEnveloppeQuery The current query, for fluid interface
     */
    public function joinCommonFichierEnveloppe($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('CommonFichierEnveloppe');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'CommonFichierEnveloppe');
        }
    ...
    /**
     * Use the CommonFichierEnveloppe relation CommonFichierEnveloppe object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   CommonFichierEnveloppeQuery A secondary query class using the current class as primary query
     */
    public function useCommonFichierEnveloppeQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ->joinCommonFichierEnveloppe($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'CommonFichierEnveloppe', 'CommonFichierEnveloppeQuery');
    }
