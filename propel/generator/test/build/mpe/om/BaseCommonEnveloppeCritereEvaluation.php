...
abstract class BaseCommonEnveloppeCritereEvaluation extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonEnveloppeItemCritereEvaluation[] Collection to store aggregation of CommonEnveloppeItemCritereEvaluation objects.
     */
    protected $collCommonEnveloppeItemCritereEvaluations;
    protected $collCommonEnveloppeItemCritereEvaluationsPartial;
     ...
     */
    protected $commonEnveloppeItemCritereEvaluationsScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collCommonEnveloppeItemCritereEvaluations = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion !== null) {
                if (!$this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion->isEmpty()) {
                    foreach ($this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion as $commonEnveloppeItemCritereEvaluation) {
                        // need to save related object because we set the relation to null
                        $commonEnveloppeItemCritereEvaluation->save($con);
                    }
                    $this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion = null;
                }
            }

            if ($this->collCommonEnveloppeItemCritereEvaluations !== null) {
                foreach ($this->collCommonEnveloppeItemCritereEvaluations as $referrerFK) {
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collCommonEnveloppeItemCritereEvaluations) {
                $result['CommonEnveloppeItemCritereEvaluations'] = $this->collCommonEnveloppeItemCritereEvaluations->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonEnveloppeItemCritereEvaluations() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonEnveloppeItemCritereEvaluation($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('CommonEnveloppeItemCritereEvaluation' == $relationName) {
            $this->initCommonEnveloppeItemCritereEvaluations();
        }
    }

    /**
     * Clears out the collCommonEnveloppeItemCritereEvaluations collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonEnveloppeCritereEvaluation The current object (for fluent API support)
     * @see        addCommonEnveloppeItemCritereEvaluations()
     */
    public function clearCommonEnveloppeItemCritereEvaluations()
    {
        $this->collCommonEnveloppeItemCritereEvaluations = null; // important to set this to null since that means it is uninitialized
        $this->collCommonEnveloppeItemCritereEvaluationsPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonEnveloppeItemCritereEvaluations collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonEnveloppeItemCritereEvaluations($v = true)
    {
        $this->collCommonEnveloppeItemCritereEvaluationsPartial = $v;
    }

    /**
     * Initializes the collCommonEnveloppeItemCritereEvaluations collection.
     *
     * By default this just sets the collCommonEnveloppeItemCritereEvaluations collection to an empty array (like clearcollCommonEnveloppeItemCritereEvaluations());
     ...
     * @return void
     */
    public function initCommonEnveloppeItemCritereEvaluations($overrideExisting = true)
    {
        if (null !== $this->collCommonEnveloppeItemCritereEvaluations && !$overrideExisting) {
            return;
        }
        $this->collCommonEnveloppeItemCritereEvaluations = new PropelObjectCollection();
        $this->collCommonEnveloppeItemCritereEvaluations->setModel('CommonEnveloppeItemCritereEvaluation');
    }

    /**
     * Gets an array of CommonEnveloppeItemCritereEvaluation objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this CommonEnveloppeCritereEvaluation is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonEnveloppeItemCritereEvaluation[] List of CommonEnveloppeItemCritereEvaluation objects
     * @throws PropelException
     */
    public function getCommonEnveloppeItemCritereEvaluations($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonEnveloppeItemCritereEvaluationsPartial && !$this->isNew();
        if (null === $this->collCommonEnveloppeItemCritereEvaluations || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonEnveloppeItemCritereEvaluations) {
                // return empty collection
                $this->initCommonEnveloppeItemCritereEvaluations();
            } else {
                $collCommonEnveloppeItemCritereEvaluations = CommonEnveloppeItemCritereEvaluationQuery::create(null, $criteria)
                    ->filterByCommonEnveloppeCritereEvaluation($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonEnveloppeItemCritereEvaluationsPartial && count($collCommonEnveloppeItemCritereEvaluations)) {
                      $this->initCommonEnveloppeItemCritereEvaluations(false);

                      foreach ($collCommonEnveloppeItemCritereEvaluations as $obj) {
                        if (false == $this->collCommonEnveloppeItemCritereEvaluations->contains($obj)) {
                          $this->collCommonEnveloppeItemCritereEvaluations->append($obj);
                        }
                      }

                      $this->collCommonEnveloppeItemCritereEvaluationsPartial = true;
                    }

                    $collCommonEnveloppeItemCritereEvaluations->getInternalIterator()->rewind();

                    return $collCommonEnveloppeItemCritereEvaluations;
                }

                if ($partial && $this->collCommonEnveloppeItemCritereEvaluations) {
                    foreach ($this->collCommonEnveloppeItemCritereEvaluations as $obj) {
                        if ($obj->isNew()) {
                            $collCommonEnveloppeItemCritereEvaluations[] = $obj;
                        }
                    }
                }

                $this->collCommonEnveloppeItemCritereEvaluations = $collCommonEnveloppeItemCritereEvaluations;
                $this->collCommonEnveloppeItemCritereEvaluationsPartial = false;
            }
        }

        return $this->collCommonEnveloppeItemCritereEvaluations;
    }

    /**
     * Sets a collection of CommonEnveloppeItemCritereEvaluation objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonEnveloppeItemCritereEvaluations A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonEnveloppeCritereEvaluation The current object (for fluent API support)
     */
    public function setCommonEnveloppeItemCritereEvaluations(PropelCollection $commonEnveloppeItemCritereEvaluations, PropelPDO $con = null)
    {
        $commonEnveloppeItemCritereEvaluationsToDelete = $this->getCommonEnveloppeItemCritereEvaluations(new Criteria(), $con)->diff($commonEnveloppeItemCritereEvaluations);


        $this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion = $commonEnveloppeItemCritereEvaluationsToDelete;

        foreach ($commonEnveloppeItemCritereEvaluationsToDelete as $commonEnveloppeItemCritereEvaluationRemoved) {
            $commonEnveloppeItemCritereEvaluationRemoved->setCommonEnveloppeCritereEvaluation(null);
        }

        $this->collCommonEnveloppeItemCritereEvaluations = null;
        foreach ($commonEnveloppeItemCritereEvaluations as $commonEnveloppeItemCritereEvaluation) {
            $this->addCommonEnveloppeItemCritereEvaluation($commonEnveloppeItemCritereEvaluation);
        }

        $this->collCommonEnveloppeItemCritereEvaluations = $commonEnveloppeItemCritereEvaluations;
        $this->collCommonEnveloppeItemCritereEvaluationsPartial = false;
    ...
    /**
     * Returns the number of related CommonEnveloppeItemCritereEvaluation objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonEnveloppeItemCritereEvaluation objects.
     * @throws PropelException
     */
    public function countCommonEnveloppeItemCritereEvaluations(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonEnveloppeItemCritereEvaluationsPartial && !$this->isNew();
        if (null === $this->collCommonEnveloppeItemCritereEvaluations || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonEnveloppeItemCritereEvaluations) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonEnveloppeItemCritereEvaluations());
            }
            $query = CommonEnveloppeItemCritereEvaluationQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonEnveloppeItemCritereEvaluations);
    }

    /**
     * Method called to associate a CommonEnveloppeItemCritereEvaluation object to this object
     * through the CommonEnveloppeItemCritereEvaluation foreign key attribute.
     *
     * @param   CommonEnveloppeItemCritereEvaluation $l CommonEnveloppeItemCritereEvaluation
     * @return CommonEnveloppeCritereEvaluation The current object (for fluent API support)
     */
    public function addCommonEnveloppeItemCritereEvaluation(CommonEnveloppeItemCritereEvaluation $l)
    {
        if ($this->collCommonEnveloppeItemCritereEvaluations === null) {
            $this->initCommonEnveloppeItemCritereEvaluations();
            $this->collCommonEnveloppeItemCritereEvaluationsPartial = true;
        }
        if (!in_array($l, $this->collCommonEnveloppeItemCritereEvaluations->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonEnveloppeItemCritereEvaluation($l);
        }
    ...
    /**
     * @param	CommonEnveloppeItemCritereEvaluation $commonEnveloppeItemCritereEvaluation The commonEnveloppeItemCritereEvaluation object to add.
     */
    protected function doAddCommonEnveloppeItemCritereEvaluation($commonEnveloppeItemCritereEvaluation)
    {
        $this->collCommonEnveloppeItemCritereEvaluations[]= $commonEnveloppeItemCritereEvaluation;
        $commonEnveloppeItemCritereEvaluation->setCommonEnveloppeCritereEvaluation($this);
    }

    /**
     * @param	CommonEnveloppeItemCritereEvaluation $commonEnveloppeItemCritereEvaluation The commonEnveloppeItemCritereEvaluation object to remove.
     * @return CommonEnveloppeCritereEvaluation The current object (for fluent API support)
     */
    public function removeCommonEnveloppeItemCritereEvaluation($commonEnveloppeItemCritereEvaluation)
    {
        if ($this->getCommonEnveloppeItemCritereEvaluations()->contains($commonEnveloppeItemCritereEvaluation)) {
            $this->collCommonEnveloppeItemCritereEvaluations->remove($this->collCommonEnveloppeItemCritereEvaluations->search($commonEnveloppeItemCritereEvaluation));
            if (null === $this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion) {
                $this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion = clone $this->collCommonEnveloppeItemCritereEvaluations;
                $this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion->clear();
            }
            $this->commonEnveloppeItemCritereEvaluationsScheduledForDeletion[]= clone $commonEnveloppeItemCritereEvaluation;
            $commonEnveloppeItemCritereEvaluation->setCommonEnveloppeCritereEvaluation(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonEnveloppeItemCritereEvaluations) {
                foreach ($this->collCommonEnveloppeItemCritereEvaluations as $o) {
        ...
        if ($this->collCommonEnveloppeItemCritereEvaluations instanceof PropelCollection) {
            $this->collCommonEnveloppeItemCritereEvaluations->clearIterator();
        }
        $this->collCommonEnveloppeItemCritereEvaluations = null;
    }
