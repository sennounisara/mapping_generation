...
abstract class BaseCommonPassationConsultation extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|CommonPassationMarcheAVenir[] Collection to store aggregation of CommonPassationMarcheAVenir objects.
     */
    protected $collCommonPassationMarcheAVenirs;
    protected $collCommonPassationMarcheAVenirsPartial;
     ...
     */
    protected $commonPassationMarcheAVenirsScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collCommonPassationMarcheAVenirs = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->commonPassationMarcheAVenirsScheduledForDeletion !== null) {
                if (!$this->commonPassationMarcheAVenirsScheduledForDeletion->isEmpty()) {
                    //the foreign key is flagged as `CASCADE`, so we delete the items
                    CommonPassationMarcheAVenirQuery::create()
                        ->filterByPrimaryKeys($this->commonPassationMarcheAVenirsScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->commonPassationMarcheAVenirsScheduledForDeletion = null;
                }
            }

            if ($this->collCommonPassationMarcheAVenirs !== null) {
                foreach ($this->collCommonPassationMarcheAVenirs as $referrerFK) {
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collCommonPassationMarcheAVenirs) {
                $result['CommonPassationMarcheAVenirs'] = $this->collCommonPassationMarcheAVenirs->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCommonPassationMarcheAVenirs() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCommonPassationMarcheAVenir($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('CommonPassationMarcheAVenir' == $relationName) {
            $this->initCommonPassationMarcheAVenirs();
        }
    }

    /**
     * Clears out the collCommonPassationMarcheAVenirs collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return CommonPassationConsultation The current object (for fluent API support)
     * @see        addCommonPassationMarcheAVenirs()
     */
    public function clearCommonPassationMarcheAVenirs()
    {
        $this->collCommonPassationMarcheAVenirs = null; // important to set this to null since that means it is uninitialized
        $this->collCommonPassationMarcheAVenirsPartial = null;

        return $this;
    }

    /**
     * reset is the collCommonPassationMarcheAVenirs collection loaded partially
     *
     * @return void
     */
    public function resetPartialCommonPassationMarcheAVenirs($v = true)
    {
        $this->collCommonPassationMarcheAVenirsPartial = $v;
    }

    /**
     * Initializes the collCommonPassationMarcheAVenirs collection.
     *
     * By default this just sets the collCommonPassationMarcheAVenirs collection to an empty array (like clearcollCommonPassationMarcheAVenirs());
     ...
     * @return void
     */
    public function initCommonPassationMarcheAVenirs($overrideExisting = true)
    {
        if (null !== $this->collCommonPassationMarcheAVenirs && !$overrideExisting) {
            return;
        }
        $this->collCommonPassationMarcheAVenirs = new PropelObjectCollection();
        $this->collCommonPassationMarcheAVenirs->setModel('CommonPassationMarcheAVenir');
    }

    /**
     * Gets an array of CommonPassationMarcheAVenir objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     * If this CommonPassationConsultation is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|CommonPassationMarcheAVenir[] List of CommonPassationMarcheAVenir objects
     * @throws PropelException
     */
    public function getCommonPassationMarcheAVenirs($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCommonPassationMarcheAVenirsPartial && !$this->isNew();
        if (null === $this->collCommonPassationMarcheAVenirs || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCommonPassationMarcheAVenirs) {
                // return empty collection
                $this->initCommonPassationMarcheAVenirs();
            } else {
                $collCommonPassationMarcheAVenirs = CommonPassationMarcheAVenirQuery::create(null, $criteria)
                    ->filterByCommonPassationConsultation($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCommonPassationMarcheAVenirsPartial && count($collCommonPassationMarcheAVenirs)) {
                      $this->initCommonPassationMarcheAVenirs(false);

                      foreach ($collCommonPassationMarcheAVenirs as $obj) {
                        if (false == $this->collCommonPassationMarcheAVenirs->contains($obj)) {
                          $this->collCommonPassationMarcheAVenirs->append($obj);
                        }
                      }

                      $this->collCommonPassationMarcheAVenirsPartial = true;
                    }

                    $collCommonPassationMarcheAVenirs->getInternalIterator()->rewind();

                    return $collCommonPassationMarcheAVenirs;
                }

                if ($partial && $this->collCommonPassationMarcheAVenirs) {
                    foreach ($this->collCommonPassationMarcheAVenirs as $obj) {
                        if ($obj->isNew()) {
                            $collCommonPassationMarcheAVenirs[] = $obj;
                        }
                    }
                }

                $this->collCommonPassationMarcheAVenirs = $collCommonPassationMarcheAVenirs;
                $this->collCommonPassationMarcheAVenirsPartial = false;
            }
        }

        return $this->collCommonPassationMarcheAVenirs;
    }

    /**
     * Sets a collection of CommonPassationMarcheAVenir objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $commonPassationMarcheAVenirs A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return CommonPassationConsultation The current object (for fluent API support)
     */
    public function setCommonPassationMarcheAVenirs(PropelCollection $commonPassationMarcheAVenirs, PropelPDO $con = null)
    {
        $commonPassationMarcheAVenirsToDelete = $this->getCommonPassationMarcheAVenirs(new Criteria(), $con)->diff($commonPassationMarcheAVenirs);


        $this->commonPassationMarcheAVenirsScheduledForDeletion = $commonPassationMarcheAVenirsToDelete;

        foreach ($commonPassationMarcheAVenirsToDelete as $commonPassationMarcheAVenirRemoved) {
            $commonPassationMarcheAVenirRemoved->setCommonPassationConsultation(null);
        }

        $this->collCommonPassationMarcheAVenirs = null;
        foreach ($commonPassationMarcheAVenirs as $commonPassationMarcheAVenir) {
            $this->addCommonPassationMarcheAVenir($commonPassationMarcheAVenir);
        }

        $this->collCommonPassationMarcheAVenirs = $commonPassationMarcheAVenirs;
        $this->collCommonPassationMarcheAVenirsPartial = false;
    ...
    /**
     * Returns the number of related CommonPassationMarcheAVenir objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related CommonPassationMarcheAVenir objects.
     * @throws PropelException
     */
    public function countCommonPassationMarcheAVenirs(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collCommonPassationMarcheAVenirsPartial && !$this->isNew();
        if (null === $this->collCommonPassationMarcheAVenirs || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCommonPassationMarcheAVenirs) {
            ...
            if ($partial && !$criteria) {
                return count($this->getCommonPassationMarcheAVenirs());
            }
            $query = CommonPassationMarcheAVenirQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collCommonPassationMarcheAVenirs);
    }

    /**
     * Method called to associate a CommonPassationMarcheAVenir object to this object
     * through the CommonPassationMarcheAVenir foreign key attribute.
     *
     * @param   CommonPassationMarcheAVenir $l CommonPassationMarcheAVenir
     * @return CommonPassationConsultation The current object (for fluent API support)
     */
    public function addCommonPassationMarcheAVenir(CommonPassationMarcheAVenir $l)
    {
        if ($this->collCommonPassationMarcheAVenirs === null) {
            $this->initCommonPassationMarcheAVenirs();
            $this->collCommonPassationMarcheAVenirsPartial = true;
        }
        if (!in_array($l, $this->collCommonPassationMarcheAVenirs->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCommonPassationMarcheAVenir($l);
        }
    ...
    /**
     * @param	CommonPassationMarcheAVenir $commonPassationMarcheAVenir The commonPassationMarcheAVenir object to add.
     */
    protected function doAddCommonPassationMarcheAVenir($commonPassationMarcheAVenir)
    {
        $this->collCommonPassationMarcheAVenirs[]= $commonPassationMarcheAVenir;
        $commonPassationMarcheAVenir->setCommonPassationConsultation($this);
    }

    /**
     * @param	CommonPassationMarcheAVenir $commonPassationMarcheAVenir The commonPassationMarcheAVenir object to remove.
     * @return CommonPassationConsultation The current object (for fluent API support)
     */
    public function removeCommonPassationMarcheAVenir($commonPassationMarcheAVenir)
    {
        if ($this->getCommonPassationMarcheAVenirs()->contains($commonPassationMarcheAVenir)) {
            $this->collCommonPassationMarcheAVenirs->remove($this->collCommonPassationMarcheAVenirs->search($commonPassationMarcheAVenir));
            if (null === $this->commonPassationMarcheAVenirsScheduledForDeletion) {
                $this->commonPassationMarcheAVenirsScheduledForDeletion = clone $this->collCommonPassationMarcheAVenirs;
                $this->commonPassationMarcheAVenirsScheduledForDeletion->clear();
            }
            $this->commonPassationMarcheAVenirsScheduledForDeletion[]= clone $commonPassationMarcheAVenir;
            $commonPassationMarcheAVenir->setCommonPassationConsultation(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collCommonPassationMarcheAVenirs) {
                foreach ($this->collCommonPassationMarcheAVenirs as $o) {
        ...
        if ($this->collCommonPassationMarcheAVenirs instanceof PropelCollection) {
            $this->collCommonPassationMarcheAVenirs->clearIterator();
        }
        $this->collCommonPassationMarcheAVenirs = null;
    }
